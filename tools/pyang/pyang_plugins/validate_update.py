...
class CheckDeviationPlugin(plugin.PyangPlugin):
    ...
    def emit(self, ctx, modules, fd):
        ...
        extensions_dir = os.path.join(str(ctx.opts.yang_dir) , 'extensions')
        extensionModulesList = list(map(lambda yn: os.path.splitext(yn)[0], os.listdir(extensions_dir)))
        ctx.errors = []

        for module in modules:            
            mod_name = module.i_modulename
            if mod_name in extensionModulesList:
                mark_deviations(module)
