...
func validate_neighbor_post_xfmr(inParams *XfmrParams) (error) {
    ...
    retVal  = validate_fields_value_post_xfmr (inParams,"BGP_NEIGHBOR","ebgp_multihop_ttl","ttl_security_hops")
    if (retVal != nil) {
       return retVal
    }
...
func validate_peer_group_post_xfmr (inParams *XfmrParams) (error) {
    ...
    retVal  = validate_fields_value_post_xfmr (inParams,"BGP_PEER_GROUP","ebgp_multihop_ttl","ttl_security_hops")
    if (retVal != nil) {
       return retVal
    }
    return nil
...
func validate_fields_value_post_xfmr (inParams *XfmrParams, tableName string, fieldName1 string, fieldName2 string) (error) {
    var field1 int64
    var field2 int64
    ...
    if dataMapDB, ok := (*inParams.dbDataMap)[db.ConfigDB][tableName]; ok {
        for key := range dataMapDB {
            ...
            if ((ok1) && (ok2)) {
                ...
                if dbErr == nil {
                    if ((!ok1) && (ok2)) {
                        fieldValStr, ok  := dbEntry.Field[fieldName1]
                        field1, _ :=  strconv.ParseInt(fieldValStr, 10, 16)
                        field2, _ =  strconv.ParseInt(fieldVal2, 10, 16)
                        if ok && ((field2 > 0 ) && (field1 >0)){
                            errStr := fieldName2 + " is not supported with " + fieldName1
                            log.Error(errStr)
                            return  tlerr.InvalidArgsError{Format:errStr}
                        }
                    } else if ((ok1) && (!ok2)) {
                        fieldValStr, ok  := dbEntry.Field[fieldName2]
                        field2, _ :=  strconv.ParseInt(fieldValStr, 10, 16)
                        field1, _ =  strconv.ParseInt(fieldVal1, 10, 16)
                        if ok && ((field1 > 0 ) && (field2 >0)){
...
func validate_prefix_limit_post_xfmr(inParams *XfmrParams, tableName string) (error) {
    ...
    if dataMapDB, ok := (*inParams.dbDataMap)[db.ConfigDB][tableName]; ok {
        for key := range dataMapDB {
            ...
            restart_str, restartOk := dataMapDB[key].Field["max_prefix_restart_interval"]
            if ((warnOk) && (restartOk)) {
                restart_val, _ :=  strconv.ParseInt(restart_str, 10, 16)
                if ((warn_str == "true") && (restart_val > 0 )) {
