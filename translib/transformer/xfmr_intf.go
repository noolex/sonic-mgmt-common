...
var rpc_get_interface_counters = func(body []byte, dbs [db.MaxDB]*db.DB) ([]byte, error) {
    ...
    if err != nil {
        log.Info("rpc_get_interface_counters: Failed to unmarshall given input data")
        result.Output.Status_detail = fmt.Sprintf("Internal parse error!")
        return json.Marshal(&result)
    }

    input, _ := mapData["sonic-counters:input"]
    mapData = input.(map[string]interface{})
    input = mapData["prefix-match"]
    input_str := fmt.Sprintf("%v", input)
    Prefix := string(input_str)
    ...
    for  ifName, oid := range ifCountInfo.Field {
        if strings.HasPrefix(ifName, Prefix) {
            ...
            prtEntry, prtErr := dbs[db.ApplDB].GetEntry(&db.TableSpec{Name:"PORT_TABLE"}, db.Key{Comp: []string{ifName}})
            if prtErr != nil {
