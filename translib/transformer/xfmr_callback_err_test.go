...
func Test_SubtreeXfmr_CRUDErr_OCYang(t *testing.T) {
	...
	/*exp_err = tlerr.InvalidArgsError{Format:"Tagged Vlan configuration: Vlan20 doesn't exist for Interface: Ethernet32"}
	exp_err = tlerr.InvalidArgsError{Format:"Invalid Vlan:Vlan20"}*/
	t.Run("SubtreeXfmr Error handling delete.", processDeleteRequest(url, true))
	time.Sleep(1 * time.Second)
...
func Test_KeyXfmr_CRUDErr_OCYang(t *testing.T) {
	...
	url = "/openconfig-interfaces:interfaces"
	url_body_json = "{\"openconfig-interfaces:interface\":[{\"name\":\"defaultEth1\",\"config\":{\"name\":\"defaultEth1\",\"type\":\"ds3\",\"mtu\":2300}}]}"
...
func Test_PostXfmr_Err_SetOCYang(t *testing.T) {
        ...
        expected_map_vlan := map[string]interface{}{"VLAN":map[string]interface{}{"Vlan5":map[string]interface{}{
                                                                                      "vlanid": "5",
                                                                                      "members@": "Ethernet32"}}}
        expected_map_vlan_member := map[string]interface{}{"VLAN_MEMBER":map[string]interface{}{"Vlan5|Ethernet32":map[string]interface{}{
                                                                                                     "tagging_mode": "tagged"}}}
        exp_err = tlerr.NotSupported("Delete not allowed at this container")
        loadConfigDB(rclient, prereq_map)
        url = "/openconfig-interfaces:interfaces/interface[name=Vlan5]/subinterfaces/subinterface[index=0]/openconfig-if-ip:ipv6/config"
        t.Run("PostXfmr Error handling delete.", processDeleteRequest(url, true, exp_err))
        time.Sleep(1 * time.Second)
        t.Run("Verify PostXfmrError handling delete - VLAN table.", verifyDbResult(rclient, "VLAN|Vlan5", expected_map_vlan, false))
        t.Run("Verify PostXfmrError handling delete - VLAN_MEMBER table.", verifyDbResult(rclient, "VLAN_MEMBER|Vlan5|Ethernet32", expected_map_vlan_member, false))
        unloadConfigDB(rclient, prereq_map)
