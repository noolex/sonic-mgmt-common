...
var YangToDb_sensor_path_key_xfmr KeyXfmrYangToDb = func(inParams XfmrParams) (string, error) {
	...
	ygXpathListInfo, ok := xYangSpecMap[raw_path]

	if !ok || ygXpathListInfo == nil {
		log.Info("ok: ", ok)
		log.Info("ygXpathListInfo: ", ygXpathListInfo)
		log.Infof("%+v", ygXpathListInfo)
		return key, tlerr.InternalError{Format: "Invalid Path", Path: inParams.uri}
	}
